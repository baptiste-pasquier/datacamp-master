...
class TreeNode(object):
    ...
    def build(self, instances, grad, hessian, shrinkage_rate, depth, param):
        ...
        if depth > param["max_depth"]:
            self.is_leaf = True
            self.weight = (
                self._calc_leaf_weight(grad, hessian, param["lambd"]) * shrinkage_rate
            )
            return
        G = np.sum(grad)
        H = np.sum(hessian)
        ...
        for feature_id in range(instances.shape[1]):
            ...
            for j in range(sorted_instance_ids.shape[0]):
                ...
                current_gain = self._calc_split_gain(
                    G, H, G_l, H_l, G_r, H_r, param["lambd"]
                )
        ...
        if best_gain < param["min_split_gain"]:
            ...
            self.left_child.build(
                instances[best_left_instance_ids],
            ...
            self.right_child.build(
                instances[best_right_instance_ids],
...
class GBT(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, valid_set=None, early_stopping_rounds=5):
        ...
        for iter_cnt in range(self.n_estimators):
            ...
            print(
                "Iter {:>3}, Train's L2: {:.10f}, Valid's L2: {}, Elapsed: {:.2f} secs".format(
                    iter_cnt, train_loss, val_loss_str, time.time() - iter_start_time
                )
            )
    ...
    def predict(self, X):
        ...
        for x in X:
            y_pred.append(
                self._predict_one(x, self.models_[: self.best_iteration_ + 1])
            )
        return y_pred
